...
class I18nGenerateMessageValidationTest < ActiveRecord::TestCase
  ...
  def test_generate_message_invalid_with_default_message
    assert_equal "is invalid", assert_deprecated { @topic.errors.generate_message(:title, :invalid, value: "title") }
  end

  def test_generate_message_invalid_with_custom_message
    assert_equal "custom message title", assert_deprecated { @topic.errors.generate_message(:title, :invalid, message: "custom message %{value}", value: "title") }
  end
  ...
  def test_generate_message_taken_with_default_message
    assert_equal "has already been taken", assert_deprecated { @topic.errors.generate_message(:title, :taken, value: "title") }
  end

  def test_generate_message_taken_with_custom_message
    assert_equal "custom message title", assert_deprecated { @topic.errors.generate_message(:title, :taken, message: "custom message %{value}", value: "title") }
  end
  ...
  test "translation for 'taken' can be overridden" do
    reset_i18n_load_path do
      ...
      assert_equal "Custom taken message", assert_deprecated { @topic.errors.generate_message(:title, :taken, value: "title") }
    end
  end
